import type { DownloadOptions, HttpOptions, HttpResponse } from './types.js';
import { createWriteStream } from 'node:fs';
import http from 'node:http';
import https, { type RequestOptions } from 'node:https';
import qs from 'node:querystring';
import { ALLOWED_PROTOCOLS, HttpMethod, MimeTypes, headers } from './constants.js';
import { buildResponse, preventDNSLookup } from './utils.js';
import { TorHttpException } from './exceptions.js';
...
export class HttpClient {
  ...
  download(url: string, options: DownloadOptions) {
    ...
    return new Promise<string>((resolve, reject) => {
      ...
      req.on('error', reject);
      req.on('timeout', () => {
        reject(new TorHttpException('Download timeout'));
      });
      req.end();
    });
  }
